...
double k[4]={  -3.1623  , 57.1757  , -1.9815  ,  7.5060};
...
void loop() 
{
  ...
  long StartingTheta=getPositionSPI(ENC_1, RES14);
  delay(msDelay);
  ...
  uint16_t AlphaRead=0;
  uint16_t ThetaRead=0;
  ...
  while(1)
  {
    ...
    while (alpha == 0xFFFF && ++attempts < 3)
    {
      AlphaRead = getPositionSPI(ENC_0, RES14); //try again
      alpha = (long)AlphaRead-start_alpha;
    }

    if (alpha == 0xFFFF) //position is bad, let the user know how many times we tried
    {
    ...
    else //position was good, print to serial stream
    {
      ...
      //Serial.print(alpha, DEC); //print the position in decimal format
      //Serial.write(NEWLINE);
    }
    ...
    while (theta == 0xFFFF && ++attempts < 3)
    {
      ThetaRead = getPositionSPI(ENC_1, RES14); //try again
      theta = ThetaRead-StartingTheta;
    }
    //Serial.println(theta*360/16384);
    //Serial.println(alpha*360/16400);
    if (theta == 0xFFFF) //position is bad, let the user know how many times we tried
    {
    ...
    else //position was good, print to serial stream
    {
     ...
     Output = -0.5*(k[0]*theta + k[1]*alpha + k[2]*dTheta + k[3]*dAlpha)*360/16400;
     
     
 //     -1.0000   59.0483   -0.9963    4.2415 


//
//    if(abs(Output)<120) Output=abs(Output)/Output*120;
//    if(abs(Output)>255) Output=abs(Output)/Output*255;
Serial.print("THETA: ");
Serial.print(theta, DEC); //print the position in decimal format
Serial.write(NEWLINE);
Serial.print("ALPHA: ");
...
double map(double val, int M1, int M2, int O1, int O2)
{
  if(val<M1) return -254;
  if(val>M2) return 254;
  double result = (val-M1)/((M2-M1)*1.0);
  result = result*(O2-O1) + O1;
  return (int)result;
}


float filter(long input, double alpha) {
    static long output = 0;                        // Output value from previous iteration, initialized to 0
    output = alpha * output + (1 - alpha) * input;  // Filter input
    return (long)output;
}
